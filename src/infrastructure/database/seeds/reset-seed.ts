...
async function resetSeed(): Promise<void> {
  ...
  try {
    ...
    await AppDataSource.query(
      // Uncomment to reset all tables
      // 'TRUNCATE TABLE "payments", "orders", "our_works", "testimonials", "faqs", "services", "users" RESTART IDENTITY CASCADE',

      // Disabled tables (to match run-seed): payments, orders, our_works, faqs, services, users
      'TRUNCATE TABLE "testimonials", "users" RESTART IDENTITY CASCADE',
    );
